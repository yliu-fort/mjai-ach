"""Generate the single-factor A/B notebooks (config + training + visualization).

Two one-click notebooks on liar's dice, mirror self-play, each isolating ONE
axis (backed by tools/ab_factor_probe.py — nothing reimplemented, AGENTS.md §7):

  notebooks/ppo_vs_ach_liars_dice1.ipynb   the ALGORITHM axis: best-practice PPO
      (its own Adam / advantage-norm / clip / multi-epoch protocol) vs
      single-factor PPO (theta=0 on the ACH scaffold) vs paper-faithful ACH.
  notebooks/sgd_vs_adam_liars_dice1.ipynb  the OPTIMIZER axis, ACH held fixed:
      constant-LR SGD vs Adam at 1e-3 and 3e-4.

Run: uv run python tools/build_ab_factor_notebooks.py
"""
...
GAME = "liars_dice1"
TOTAL = 10_000  # per-arm probe budget (liars_dice1 default; eval is the cost driver)
EVAL_EVERY = 2_500

# One entry per generated notebook. arms = (label, base-config stem, overrides, blurb).
# label is the output subdir and the figure legend; overrides is the single axis
# the A/B varies on top of the base YAML.
NOTEBOOKS: dict[str, dict[str, object]] = {
    "ppo_vs_ach": {
        ...
        "arms": [
            (
                "ppo_tuned",
                "liars_dice1_ppo_tuned_mlp_mirror",
                {},
                "最佳实践 PPO（Adam + 优势归一化 + grad-clip 0.5 + 4 epochs + lr 3e-4）",
            ),
            (
                "ppo_single",
                "liars_dice1_ppo_mlp_mirror",
                {},
                "单因子 PPO：PPO 策略项跑在 ACH 脚手架上（theta=0）",
            ),
            ("ach", "liars_dice1_ach_mlp_mirror", {}, "论文忠实 ACH（theta=1）"),
        ],
        "intro": "三臂对照：**最佳实践 PPO**（按 PPO 自己的 37-details 协议）、**单因子 "
        "PPO**（theta=0，只换策略项、脚手架仍是 ACH 的 SGD/原始优势/单 epoch/无裁剪）、"
        "**论文 ACH**（theta=1）。`ppo_tuned` 与 `ach` 的差是「两个算法各按作者本意」的整体差；"
        "`ppo_single` 与 `ach` 的差是「只有策略算子不同」——两者一起看，就能把 PPO↔ACH 的差异"
        "拆成「策略算子」和「其余协议（优化器/优势/epoch/裁剪）」两部分。`ppo_single`/`ach` "
        "分别与仓库现有 `configs/exp/liars_dice1_ppo_mlp_mirror.yaml`/`..._ach_...` 逐位一致"
        "（也就是 theta 扫描的两端）。",
        "footer": "- **`ppo_tuned` vs `ach`**：整体算法差。若 `ppo_tuned` 明显更差/发散，"
        "说明即便给足 PPO 最佳实践，ACH 在这个博弈上仍占优（论文动机的强版本）。\n"
        "- **`ppo_single` vs `ach`**：单因子（只差策略项）。这条等价于 theta 扫描的 0↔1 两端，"
        "是「纯策略算子」的贡献。\n"
        "- **`ppo_tuned` vs `ppo_single`**：把「PPO 的最佳实践加成」单独量出来——两者都是 "
        "theta=0 的 clipped surrogate，只差 Adam/优势归一化/裁剪/多 epoch 这一束脚手架旋钮。\n"
        "- 遥测格里 `clip_frac` 只有 theta=0 的两臂有值，`gate_off_frac` 只有 `ach` 有值；"
        "`grad_norm` 上 `ppo_tuned`（Adam）与另两臂（SGD）不在一个量级是正常的。",
    },
    "sgd_vs_adam": {
        "title": "ACH-SGD vs ACH-Adam",
        "axis": "优化器",
        "root": "nb_sgd_adam",
        "arms": [
            ("ach_sgd", "liars_dice1_ach_mlp_mirror", {}, "ACH + 恒定 lr SGD @ 1e-3（论文 H.3）"),
            (
                "ach_adam_1e-3",
                "liars_dice1_ach_mlp_mirror",
                {"optimizer": "adam", "learning_rate": 0.001},
                "ACH + Adam @ 1e-3（会打印 ACHFidelityWarning）",
            ),
            (
                "ach_adam_3e-4",
                "liars_dice1_ach_mlp_mirror",
                {"optimizer": "adam", "learning_rate": 0.0003},
                "ACH + Adam @ 3e-4（会打印 ACHFidelityWarning）",
            ),
        ],
        "intro": "算法固定为**论文忠实 ACH（theta=1）**，只切**优化器**：论文的恒定学习率 SGD "
        "(lr=1e-3) vs Adam 两个学习率（1e-3 严格单因子；3e-4 是 PPO 37-details 给 Adam 的推荐值，"
        "对 Adam 更公平但同时动了 lr）。**ACH+Adam 偏离论文 SGD 协议，会如实打印 "
        "`ACHFidelityWarning`——这是预期正确行为，不是报错。** 三臂用同一份 "
        "`configs/exp/liars_dice1_ach_mlp_mirror.yaml`，只在 notebook 里覆写 optimizer/lr，"
        "覆写会写进各臂的 `config.json`（可追溯，AGENTS.md §9）。",
        "footer": "- **`ach_sgd` vs `ach_adam_1e-3`**：严格单因子——只有优化器不同。Adam@1e-3 "
        "常常太激进甚至发散，那本身就是结论（ACH 的 SGD 协议不是随手可换的）。\n"
        "- **`ach_adam_3e-4`**：给 Adam 它自己该用的 lr。若它追平/超过 SGD，说明「ACH 必须配 "
        "SGD」更多是 lr 没配对，而非 Adam 本身不行；若仍差，则是优化器本身的差异。\n"
        "- 遥测格里 `grad_norm`：Adam 会把梯度重标定到 O(1) 附近，SGD 则跟着 ACH 的无界 "
        "1/pi_old 抖动——两者不在一个量级是正常的（log 轴）。`clip_frac` 三臂都空（theta=1）。",
    },
}
...
def arms_literal(arms: list[tuple]) -> str:
    """Render the arm list as a readable Python literal for the parameter cell."""
    lines = ["ARMS = ["]
    for label, config, overrides, blurb in arms:
        lines.append(f"    {{'label': {label!r}, 'config': {config!r},")
        lines.append(f"     'overrides': {overrides!r}, 'blurb': {blurb!r}}},")
    lines.append("]")
    return "\n".join(lines)


HEADER_MD = """# A/B 验证 · {game}（{title}）

一键 notebook：配置 → 训练 → 可视化。与 `notebooks/ab_{game}.ipynb`（mirror vs
league）、`notebooks/theta_{game}.ipynb`（theta 扫描）同族——这里固定 **mirror 自
博弈**，只沿**{axis}**这一个轴做 A/B。
...
- 输出目录：`runs/{root}/{game}/<label>/seed_N/`。**臂按配置 hash 缓存**（不是按目录
...
from pathlib import Path
OUT_ROOT = Path("runs/{root}") / GAME
FIG_TITLE = "{game}: {title} — exploitability vs env-steps (lower = closer to Nash)"'''
...
FIGURE_CODE = """# === Comparison figure (mean + min-max band across seeds, one colour per arm) ===
...
TELEMETRY_CODE = """# === Per-update telemetry (downsampled read: grad scale / gate / clip) ===
...
曲线为 mean + min–max 带（n={n_seeds} seeds），同预算下谁低谁好；单 seed 发散会把带
撑开，这本身就是信息。
...
def build(key: str, spec: dict[str, object]) -> Path:
    CELLS.clear()
    arms = spec["arms"]  # type: ignore[assignment]
    md(
        HEADER_MD.format(
            game=GAME,
            title=spec["title"],
    ...
    code(
        PARAMS_CODE.format(
            game=GAME,
            arms_literal=arms_literal(arms),  # type: ignore[arg-type]
            total=TOTAL,
            eval_every=EVAL_EVERY,
            root=spec["root"],
            title=spec["title"],
    ...
    md(POLICY_MD)
    code(POLICY_CODE)
    md(FOOTER_MD.format(n_seeds=4, footer=spec["footer"]))
    nb = {
        "cells": list(CELLS),
        "metadata": {
            "kernelspec": {
    ...
    out = OUT_DIR / f"{key}_{GAME}.ipynb"
