...
code(
    ...
    "    )\n",
    "    print(f'\\n=== {game}/{algo}/{mode} ({N_STEPS_LOCAL} steps) ===', flush=True)\n",
...
code(
    "# Both panels are drawn into ONE figure (plot_brps_trajectory takes `ax`),\n",
    "# then shown once. Calling plt.show() inside the loop would flush the still-\n",
    "# empty grid as its own image and close it, so the composite never appeared.\n",
    ...
    "    plot_brps_trajectory(rows, title=f'BRPS - {algo}/{mode}', ax=ax)\n",
    "    # standalone per-cell copy on disk (AGENTS.md §6)\n",
    "    plots_dir = run_dirs[key] / 'plots'; plots_dir.mkdir(exist_ok=True)\n",
    "    side = plot_brps_trajectory(rows, title=f'BRPS - {algo}/{mode}',\n",
...
code(
    ...
    "    if not curves:\n",
    "        ax.axis('off'); continue\n",
    "    plot_equilibrium_curves(curves, title=game, ax=ax)   # into THIS panel\n",
    "    # standalone per-game copy on disk (AGENTS.md §6)\n",
    "    plots_dir = run_dirs.get((game, 'ach', 'mirror'), list(run_dirs.values())[0]) / 'plots'\n",
    "    plots_dir.mkdir(exist_ok=True)\n",
    "    side = plot_equilibrium_curves(curves, title=game,\n",
    "                                   save_path=plots_dir / f'fig2_{game}.png')\n",
    "    plt.close(side)\n",
    "for j in range(len(games_run), nrows * ncols):\n",
    "    axes[j // ncols][j % ncols].axis('off')\n",
    "fig.tight_layout()\n",
    "plt.show()   # one composite image, drawn once",
)
