...
> **2026-07-26 修订（比较口径）**：上述三条最初记录的是「逐比特相同」，那是在
> 生成该 fixture 的那台机器上实测的结果。float32 只在同一平台内可逐比特复现：
> 换一个 BLAS / SIMD 宽度 / FMA 收缩 / torch 构建，同一个表达式的求和结合顺序
> 就会变，末位随之改变。因此比较口径改为容差（`rel_tol=1e-6`，约 8 个 float32
> ulp；详见 AGENTS.md §5 与 `tools/gen_nn_golden.py` 顶部的实测数据）。这不放松
> D11 的证据强度：实测跨平台噪声 ≤4.4 ulp，而这些场景能表达的**最轻微的真实
> 数值改动**（`eta` 变动 0.1%）就已经移动 1.2e-03，两者相差三个数量级。
