...
**未验证的两点**（下一步该测的就是这两点）：
1. 上式的 `~` 是逐信息态的近似——真实的每信息态权重是 `sum_{h in I} (reach h)^(1-κ)`，
   与 `rho(I)^(1-κ)` 在**同一信息态含多条历史时**不等（liars 里一个信息态含 6 条，
   差在对手骰子上）。需要逐历史计算来核实，本次**未做**。
2. 方差：`reach^(-0.5)` 可达 1e5 量级，与既有的 `iw_clip=20` 叠加。需要截断，并用
   §8.1 的 effN 口径确认截断后权重分布仍落在目标区间。
