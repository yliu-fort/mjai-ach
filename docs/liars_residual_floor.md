# Liar's Dice 残余地板（0.146）的根因：on-policy 访问分布，不是估计误差
...
>
> **根因：ACH 从 on-policy rollout 学习，因此每个信息态拿到的梯度权重正比于
> ρ(I)＝一局自对弈访问它的概率；而最佳响应可以自由地把博弈导向策略几乎不去的信息态。
> 在 Liar's Dice 上这两个权重差 5.5 倍，且 ρ 的中位数是 4.7e-10 —— 97% 的信息态实际
> 收不到任何梯度。**
>
> - **相关证据**：失配比在三个游戏上与（收益归一化的）地板**单调同序**：
>   Kuhn 1.02× / Leduc 3.84× / liars 5.50×。
...
## 2. 失配的测量：exploitability 藏在训练看不见的地方

`tools/reach_mismatch.py` 对同一个策略算两种加权下的可改进量：

```
regret(I) = max_a A(I,a)
R_sampled = sum_I rho(I)     * regret(I)     <- ACH 的梯度看得见的
R_counter = sum_I cfreach(I) * regret(I)     <- 最佳响应收得走的
```

对三个游戏各自训练好的策略（liars 用 `ab_fix/..._cap256_seed0` 末 checkpoint，
Kuhn/Leduc 用 `avg_anchor/*_seed0` 末 checkpoint）：

| 游戏 | \|S\| | Δ（收益幅度） | expl | **expl/Δ** | **失配比 R_counter/R_sampled** | ρ 中位数 | 自身不可达 | bottom 1% 访问量持有的 regret | bottom 10% |
|---|---|---|---|---|---|---|---|---|---|
| Kuhn | 12 | 4.0 | 0.0349 | **0.0087** | **1.02×** | 1.8e-1 | 0 | 1.6% | 5.6% |
| Leduc | 936 | 26.0 | 0.4892 | **0.0188** | **3.84×** | 6.6e-4 | 0 | 14.6% | 59.3% |
| **Liar's Dice** | **24576** | 2.0 | 0.1464 | **0.0732** | **5.50×** | **4.7e-10** | **13196 (54%)** | **33.8%** | **74.2%** |

三条读数：

1. **收益归一化后的地板与失配比单调同序**（0.0087 / 0.0188 / 0.0732 对
   1.02× / 3.84× / 5.50×）。Kuhn 的失配比 1.02 意味着**几乎没有失配**——而 Kuhn
   正是 ACH 复现得最好的游戏。
2. **liars 的访问分布是退化的**：中位数 ρ = 4.7e-10，97.2% 的信息态 ρ < 1e-4，
   **13196 个（54%）信息态的自身 reach 实际为零**。
3. **exploitability 恰恰藏在训练不去的地方**：只占全部访问量 **1%** 的那批信息态，
   持有 **33.8%** 的反事实 regret；占 10% 的持有 74.2%。Kuhn 的同一数字是 1.6% / 5.6%。

**失配是博弈树的性质，不是训练造出来的**：在**未训练的均匀策略**上失配比就已经是
...
> 靠外推填出来的信息态（§2 第 3 条：1% 的访问量持有 33.8% 的 regret）。
...
### 6.2 一个**未解释**的旁支观察（如实记录）

在**欠训练**的低成本配置（width 256、2000 epoch、无 L-BFGS）上扫四种权重，2 个 seed：

| 加权 | seed 0 | seed 1 | 权重 > 1e-6 的信息态 |
|---|---|---|---|
| uniform | 0.1565 | 0.1682 | 100% |
| reach | 0.1869 | 0.2432 | 22.2% |
| **sqrt(ρ)** | **0.0092** | **0.0108** | 94.3% |
| cf（反事实 reach） | 0.1750 | — | 93.3% |

- `reach` > `uniform` 的方向与 §6.1 一致（该预算下差距小，因为两臂都远未收敛）。
- **`sqrt(ρ)` 比 uniform 好 15 倍，且跨 seed 复现。** 这不是噪声。
- 我对它提出的机制假设——"exploitability 是**反事实** reach 加权的量，所以 cf 权重
  应当最优"——**被 `cf` 臂直接证伪**（0.1750，比 uniform 还差）。coverage 也不是解释
  （cf 93.3% vs sqrt 94.3%，结果差 17 倍）。
- **诚实结论：我没有已验证的机制。** 最可能的读法是这是**优化路径效应**而非渐近
  性质——在远未收敛的预算下，权重决定了"先修哪些信息态"，sqrt 恰好排序得好。要
  区分，需要在收敛配置（width 1024 + L-BFGS）上跑 sqrt，本次**未跑**。
- 这条观察**不影响** §6.1 与本报告的主结论：后者依赖的是 uniform 与 reach 在
  **同一个收敛配置**下的对照，与此处的欠训练扫描无关。

---

## 7. 局限与未做的事

1. **单 seed**：精确动力学是确定性的（无 seed），但 RL 对照列取自 seed 0 单条曲线。
   §2 的失配比测量同样只在各游戏一条轨迹的末 checkpoint 上做。
2. **reach 臂被截断**：liars 的 reach 与 lr×100 两臂分别停在 39k / 26k（算力让位）。
   结论依赖斜率外推，已在 §3 标注。
3. **uniform 加权不是一个可直接落地的算法**：任何 on-policy 采样器都交付不了它。
   实用的中间物是**探索性行为策略**（μ = (1−ε)π + ε·uniform，并把 `π_old` 记为 μ，
   使 `1/π_old` 仍精确抵消采样概率、期望梯度不变而 ρ 被展平）。**本次未实现**——
   它要动 `mlp.py` 的采样路径与 rollout 记录，属于对论文的偏离，且需要单独的
   治理决策。这是本报告指出的**下一个实验**，不是本报告的结论。
3. **Theorem 1 的第二项**（`Δ Σ_s (w_h−w_l)/w_h`，与 T 无关）在形状上与本机制吻合
   ——一个不随训练时长衰减的、由权重范围决定的项。但 `paper_spec_ach.md` 的转录未
   给出 `w_h/w_l` 的精确定义，**本报告不宣称与该式的定量对应**，只作为定性佐证。
4. **未证明 0.146 是"不动点"**：reach 加权下的表格动力学不是卡在不动点上，而是
...
**Liar's Dice 残余 0.146 地板的根因是 on-policy 访问分布与最佳响应可达分布之间的
权重失配，不是任何估计误差。**

```
博弈树深、信息态多（24576），on-policy 访问分布退化（中位 ρ = 4.7e-10）
   └─► ACH 对信息态 I 的有效学习率 = lr·ρ(I)，97% 的信息态实际收不到梯度
          └─► 最佳响应自由地攻击这些信息态（1% 的访问量持有 33.8% 的 regret）
                 └─► MLP 的跨信息态泛化把梯度外推过去，救回大部分（0.74 → 0.15）
                        └─► 外推补不完的残差 = 0.146
```

三个可检验的推论，全部实测成立：把权重展平，同预算 exploitability 降 4.8 倍（§3）；
改善幅度随失配比缩放（Kuhn 1.29× vs liars 4.8×，§3.1）；一切不改变 ρ 的旋钮
（critic / batch / 容量 / β / 门控）都无效（§5）。

**治理**：本报告只给结论、不动默认（沿 `liars_floor_ablation.md` 先例）。没有新增
任何配置默认值，`src/` 未改动，新增的三个工具都在 `tools/` 下。
