...
### 9.5 收敛到的到底是哪个策略：与精确解逐分量对比

精确解 **NE = (1/16, 10/16, 5/16) = (0.0625, 0.6250, 0.3125)**，全混合。`BR` = 最优反击动作。
`expl/50` 把 exploitability 按 BRPS 的支付量级归一（±50），这是跨博弈唯一可比的读法。

| 来源 | P_R | P_P | P_S | TV | expl | expl/50 | BR |
|---|---|---|---|---|---|---|---|
| **精确 NE** | 0.0625 | 0.6250 | 0.3125 | 0 | 0 | 0 | — |
| 精确算子 `l_th=log10/2`（无噪声最优） | 0.0625 | 0.6252 | 0.3123 | **0.0002** | **0.0023** | **0.005%** | P |
| 精确算子 `β=0.1` | 0.0629 | 0.6271 | 0.3100 | 0.0025 | 0.0227 | 0.05% | P |
| 采样代理 `η=0.02,B=1024` @1e8 末点 | 0.0627 | 0.6320 | 0.3053 | 0.0072 | 0.0400 | 0.08% | P |
| 采样代理 同上 平均策略 | 0.0648 | 0.6353 | 0.2999 | 0.0126 | 0.1194 | 0.24% | P |
| **RL `η=0.02` @1e6 平均策略** | 0.0642 | 0.6408 | 0.2951 | 0.0174 | 0.128 | 0.26% | P |
| RL `η=0.005,β=0.003` @1e6 末点 | 0.0605 | 0.6388 | 0.3007 | 0.0138 | 0.167 | 0.33% | P |
| RL `η=0.02,l_th=1.1513` @1e6 平均 | 0.0756 | 0.7441 | 0.1802 | 0.1323 | 0.990 | 1.98% | P |
| RL paper 臂 @1e6 平均策略 | 0.6520 | 0.0000 | 0.3480 | 0.6250 | 17.4 | **35%** | R |

三个读法：

1. **调好之后的策略在几何上已经很接近精确解**：TV 0.007–0.017，即每个动作 0.7–1.7 个百分点的
   概率误差；exploitability 0.04–0.13 相对 ±50 的支付尺度只有 **0.08%–0.26%**。对照 paper 臂的
   35%（一个纯策略）。所以"收敛不到 Nash"说的是**最后那 0.1–0.3%**，不是量级问题。
2. **偏离方向在所有稳定臂上一致：P 偏多、S 偏少、R 基本准，最优反击恒为 Paper。** 这是**不动点
   自身的性质**（零噪声的精确算子上就有），幅度随 `β/η` 增大。⚠️ 机制未完全推导：把它线性化成
   "熵梯度被 `M⁻¹` 旋转"只预测对 R/S 的符号、幅度差 10 倍且完全漏掉 P 那一项 —— 因为该不动点上
   门控在一个动作上是**激活**的（`gate_off_frac` 0.33），所以盒子也参与决定不动点，不只是熵。
3. **修正上一节的口径建议**：`η=0.02, B=1024` 在 1e8 步时**末点（0.040）比平均策略（0.119）更好**
   —— 大 batch 把轨道半径压到比"平均值把整段瞬态也算进来"更小。平均策略只在小 batch / 短预算下
   是更好的汇总（RL 1e6 步的 η=0.02 臂：平均 0.128 vs 末点 0.443）。两者都要报，别只报一个。
