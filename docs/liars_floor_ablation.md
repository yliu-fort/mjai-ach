...
> **续篇：残余的 0.146 已定根因 → `docs/liars_residual_floor.md`。**
> 结论是 **on-policy 访问分布与最佳响应可达分布之间的权重失配**（liars 上 5.50×，
> ρ 中位数 4.7e-10）。该报告同时**回收了两条本报告留下的"未验证"项**：B3（oracle
> 优势）与"表格化去掉函数逼近"都被 `tools/exact_ach.py` 的精确动力学一次性覆盖——
> 精确优势**不**破地板，且带完美优势的表格版比带噪的 MLP run **更差**。
