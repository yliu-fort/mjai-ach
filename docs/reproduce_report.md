...
- **U1（减均值进损失）**：**已裁决（§6 探针）**。原始-logit 臂（rawy）
  @2e6 = 0.545，低于基线带 [0.64, 0.70]，说明 Algorithm 2 字面读法略优；
  但差距量级（-0.13）不足以解释 Liar's 与论文均值的 +0.15 偏差。
...
- **U1 原始-logit（Algorithm 2 字面读法）**：2e6 探针 0.545 vs 基线带
  [0.64, 0.70]，是探针里信号最好的一臂，且有论文字面依据；可与 legalmean
  组合。**建议优先。**
- **重要性权重 1/π_old 无界**：ACH 损失含 `y(a)/π_old(a)`，而 ratio 门控在
  单线程下恒真（p28）＝不设限。策略锐化后稀有动作的 π_old 很小 → 梯度爆炸，
  与观察到的"锐化之后才失稳"高度吻合。
- **U5（λ）**：`lam1` 探针 2e6 = 0.647，落在基线带内，无信号，优先级最低。
