...
### 6.5 LayerNorm + 原始 logit：首个通过筛选的候选

**动机**（外部提出的架构假设）：论文正文写"减均值 + 单侧门控"，而门控阈值
`l_th` 要有意义，logit 就必须有稳定的尺度。减均值是**手动归一化**；另一条路
是**架构级归一化**——在 trunk 末尾加 LayerNorm，然后直接对**原始 logit**
施加 `|y| < l_th`。

**配置**：`trunk_layernorm=True` + `gate_centered_logits=False` +
`loss_centered_logits=False`（门控与损失体都用原始 logit），liars seed 1，
4e6 步筛选预算。

**结果（与论文同预算逐点对照）**：

| 步数 | LayerNorm+原始logit | 基线 seed 1 | 论文 |
|---|---|---|---|
| 0.5e6 | 0.267 | 0.558 | **0.266** |
| 1.0e6 | 0.247 | 0.638 | 0.220 |
| 2.0e6 | 0.224 | 0.664 | 0.204 |
| 3.0e6 | 0.218 | 0.616 | 0.194 |
| 4.0e6 | **0.194** | 0.608 | **0.181** |

**D5 口径（3.6–4e6 同窗口）**：我们 **0.205**，论文 mean 0.194、band
**[0.162, 0.227]** → **落在论文带内**；基线同窗口 0.571。全程**单调下降、
无任何冲高**，与论文曲线形态一致。

![LayerNorm 结果](figs/layernorm_liars.png)

**遥测线索**：`pterm_max`（单样本策略项 `η·y·c·A/π_old` 的幅值）从 legalmean
的**数百**量级降到**约 10**，`grad_norm` 稳定在 6–9。注意 `iw_max` 仍然很大
（数十至数百）——**无界的 `1/π_old` 没有被消除**，是与之相乘的 `y` 被压小了，
使乘积回到温和量级。

**⚠️ 三个必须说明的限制**：

1. **机制陈述不严格成立**。"LayerNorm → logit 尺度被限定 → `l_th` 有绝对
   意义"这一推论有漏洞：LayerNorm 后面是**自由的 policy head 线性层**
   （`logits = W·LN(h) + b`，`W`/`b` 无约束），被归一化的是**特征**而非
   logit。单状态装置实测两者 max|logit| 分别为 2.277 / 2.293，**几乎无差异**
   （该装置里限定 logit 的是门控本身）。所以本条目记录的是**经验有效性**，
   其**因果机制尚未查明**——单状态装置测不到 Liar's Dice 真正的**跨状态**
   特征漂移。
2. **这是三因子同时改动**（layernorm + 原始门控 + 原始损失体）对基线，
   各因子的贡献**未做归因**。注意 §6.4 已单独证伪"仅改原始损失体"（rawy
   全程更差），说明原始 logit 必须与归一化配套才有效，但完整归因需要
   单因子拆解。
3. **4e6 是筛选预算，不等于复现**。legalmean 的失稳始于 ~3.6e6 并恶化至
   8e6；本 run 只覆盖到该窗口起点。

#### 全长 1e7 验证结果（seed 0，三臂同 seed 配对）

| 臂 | final（末 10% 均值） | 与论文均值差 |
|---|---|---|
| 基线（论文忠实） | 0.3066 | +0.136 |
| legalmean（A5） | 0.3366 | +0.166 |
| **LayerNorm + 原始 logit** | **0.1986** | **+0.028** |
| 论文 | 0.1709 | band [0.1539, **0.1881**] |

**判定：未通过 D5**——0.1986 落在论文带外 0.0105，且 |Δmean|=0.0277 >
半宽 0.0171。但它**消掉了约 80% 的 gap**（与同 seed 基线相比 −0.1080）。

**稳定性才是最显著的差别**（3.6e6 之后，即 legalmean 的失稳窗口）：

| 臂 | max | min | std |
|---|---|---|---|
| 基线 | 0.480 | 0.300 | 0.0398 |
| legalmean | **0.893** | 0.285 | **0.1249** |
| **LayerNorm** | **0.235** | 0.177 | **0.0133** |

LayerNorm 臂全程无冲高，波动比基线小 3 倍、比 legalmean 小 9 倍，形态与
论文的平坦收敛一致。
